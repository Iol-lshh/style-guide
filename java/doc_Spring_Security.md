...
- [SpringSecurity](#springsecurity)
    - [0. 시큐리티 적용 가이드](#0-시큐리티-기초)
    - [1. gradle](#1-gradle)
    - [2. 설정 관리](#2-설정-관리)
    - [3. config](#3-config)
        - [3.1 SecurityPreConfig](#31-securitypreconfig)
            - [3.1.1 인증 매니저 AuthenticationManager](#311-인증-매니저-authenticationmanager)
        - [3.2 SecurityFilterChainConfig](#32-securityfilterchainconfig)
    - [4. filter](#4-filter)
        - [4.1 InitialAuthenticationFilter](#41-initialauthenticationfilter)
        - [4.2 JwtAuthenticationFilter](#42-jwtauthenticationfilter)
        - [4.3 AccessAuthenticationFilter](#43-accessauthenticationfilter)
        - [4.4 RefreshAuthenticationFilter](#64-refreshauthenticationfilter)
        - [4.5 RefreshLogoutFilter](#45-refreshlogoutfilter)
    - [5. provider](#5-provider)
        - [5.1 UsernamePasswordAuthenticationProvider](#51-usernamepasswordauthenticationprovider)
        - [5.2 thirdParty AuthenticationProvider](#52-thirdparty-authenticationprovider)
    - [6. SecurityService](#6-service)
        - [6.1 SecurityService](#61-securityservice)
            - [6.1.1 User CRUD](#611-user-crud)
            - [6.1.2 UserToken CRUD](#612-usertoken-crud)
            - [6.1.3 DisabledAccessToken CRUD](#613-disabledaccesstoken-crud)
        - [6.2 SecurityService 구현 가이드](#62-securityservice-구현-가이드)
            - [6.2.1 UserDto](#621-userdto)
                - [6.2.1.1 UserDelegator](#6211-userdelegator)
            - [6.2.2 User entity와 UserRepository 구현 가이드](#622-user-entity와-userrepository-구현-가이드)
            - [6.2.3 UserToken entity와 UserTokenReporisory 구현 가이드](#623-usertoken-entity와-usertokenreporisory-구현-가이드)
            - [6.2.4 DisabledAccessToken entity와 DisabledAccessToken 구현 가이드](#624-disabledaccesstoken-entity와-disabledaccesstoken-구현-가이드)
    - [7. encoder](#7-encoder)
        - [7.1 PasswordEncoder](#71-passwordencoder)
            - [7.1.0 PlainTextEncoder](#710-plaintextencoder)
            - [7.1.1 Sha512PasswordEncoder](#711-sha512passwordencoder)
    - [8. helper](#8-helper)
        - [8.1 AuthorizationHeaderHelper](#81-authorizationheaderhelper)
        - [8.2 TokenHelper](#82-tokenhelper)

# SpringSecurity
- 빠른 적용을 위한다면, [6. service](#6-service) 내용을 우선적으로 참고할 것을 추천
...
### 3.1.1 인증 매니저 AuthenticationManager
...
## 4.1 InitialAuthenticationFilter
...
## 4.3 AccessAuthenticationFilter
...
## 4.4 RefreshAuthenticationFilter
...
## 5.1 UsernamePasswordAuthenticationProvider
- 헤더의 `"Authorization": "Basic {username}:{password}"`로 인증

## 5.2 thirdParty AuthenticationProvider
...
### 6.1.3 DisabledAccessToken CRUD
...
### 6.2.2 User entity와 UserRepository 구현 가이드
...
### 6.2.3 UserToken entity와 UserTokenReporisory 구현 가이드
...
### 6.2.4 DisabledAccessToken entity와 DisabledAccessToken 구현 가이드
...
### 7.1.0 PlainTextEncoder
- 아무것도 하지 않는 encoder
- 테스트용

### 7.1.1 Sha512PasswordEncoder
- Sha512 이용

# 8. helper
- 기타 헬퍼

## 8.1 AuthorizationHeaderHelper
